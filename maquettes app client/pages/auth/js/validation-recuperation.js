// Validation récupération JavaScript
...
function initValidationPage() {
    ...
    const phoneNumber = getRecoveryPhone();
    if (phoneNumber) {
        displayPhoneNumber(phoneNumber);
    }
}

// Récupérer le numéro de téléphone de récupération
...
function initValidationForm() {
    ...
    form.addEventListener('submit', function(e) {
        e.preventDefault();
        validateRecoveryCode(codeInputs, submitBtn);
    });
...
function validateRecoveryCode(inputs, submitBtn) {
    ...
    setTimeout(() => {
        ...
        if (isValidRecoveryCode(code)) {
            // Succès - sauvegarder l'autorisation et rediriger
            localStorage.setItem('recoveryAuthorized', 'true');
            showSuccess('Identité vérifiée avec succès !');
...
// Vérifier si le code de récupération est valide (simulation)
...
function resendRecoveryCode() {
    ...
    setTimeout(() => {
        ...
        showInfo('Un nouveau code de récupération a été envoyé.');
    }, 2000);
}
